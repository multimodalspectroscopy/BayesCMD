...
def sample(n, triplet):
    ...
    if triplet[0] == 0:
        ...
        result[:] = numpy.random.lognormal(
            triplet[1], numpy.sqrt(triplet[2]), n)
    return result
...
def bcmd_proc(beta, n, params, queue, do_perturb, obj):
    ...
    seq += steps.abcAbsoluteSequence(obj.times,
                                     obj.perturb(do_perturb), obj.vars,
                                     outhead=False)
    ...
    if obj.suppress:
        ...
        succ = abortable.call([obj.program, '-i', input, '-o', output],
                              stdout=obj.DEVNULL, stderr=obj.DEVNULL,
                              timeout=obj.timeout)
    else:
        # create files to hold log values
        stdoutname = os.path.join(
            obj.workdir, '%s_%d_%d.stdout' % (obj.name, n, beta))
        stderrname = os.path.join(
            obj.workdir, '%s_%d_%d.stderr' % (obj.name, n, beta))
        try:
        ...
        # invoke the model
        succ = abortable.call([obj.program, '-i', input, '-o', output],
                              stdout=f_out, stderr=f_err, timeout=obj.timeout)
...
class model_bcmd:
    ...
    def __init__(self,
                 ...
                 # an initial step sequence in case we want to prepend stuff
                 # like like resets and steadying
                 baseSeq=[],
        ...
        # includes info about priors
        self.parameters = params
        # this is kind of daft, but ASB seems to assume species initial
        # conditions will be appended to params
        self.nparameters = len(params) + len(vars)
        ...
        self.have_perturbations = any(
            [p.get('dist', '') in ['normal', 'uniform'] for p in inputs])

        # we'll always arrange for output to match data, so default behaviour
        # should be correct
        self.fit = None
    ...
    # result is the weighted sum of distances (or just the straight sum if
    # weights aren't provided)
    ...
    # set up, call out to run the actual simulation(s), read and collate the
    # results
    def simulate(self,
                 # parameter values drawn by abcsmc (a list of n lists)
                 p,
    ...
    def writeInput(self, id_beta, id_n, params, do_perturb=True):
        ...
        seq += steps.abcAbsoluteSequence(self.times, self.perturb(
            do_perturb), self.vars, outhead=False, steady=self.steady)

        filename = os.path.join(
            self.workdir, '%s_%d_%d.input' % (self.name, id_n, id_beta))
    ...
    def run(self, id_beta, id_n, input):
        outname = os.path.join(self.workdir, '%s_%d_%d.out' %
                               (self.name, id_n, id_beta))

        if self.suppress:
            # invoke the model program as a subprocess
            succ = abortable.call([self.program, '-i', input, '-o', outname],
                                  stdout=self.DEVNULL, stderr=self.DEVNULL, timeout=self.timeout)
        else:
            stdoutname = os.path.join(
    ...
    def perturb(self, do_perturb=True):
        if do_perturb and self.have_perturbations:
            ...
            # getting identically perturbed, which rather defeats the
            # purpose...
            if self.reseed:
                numpy.random.seed()
            ...
            for input in perturbed:
                input['points'] = sample(
                    len(input['points']), translate_prior(input)) + input['points']
            return perturbed
        else:
            return self.inputs
