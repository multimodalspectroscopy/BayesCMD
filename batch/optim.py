...
CONFIG_DEFAULT = {'build': BUILD,
                  'work': None,
                  'info': INFO,
                  # it probably doesn't make sense for this to be anything other than
                  # GLP, but...
                  'job_mode': JOB_MODE,
                  'solver': SOLVER,
...
def process_args():
    ...
    ap.add_argument('--version', action='version',
                    version='optim version %.1fa' % VERSION)
    ap.add_argument(
        '-b', '--build',
    ...
    ap.add_argument(
        '-o', '--outdir',
        help='work/output directory (default:[BUILD]/[MODEL_NAME]/[TIMESTAMP]',
        metavar='DIR')
    ap.add_argument(
        '-d', '--dryrun',
...
def process_vars(vars, aliases, data):
    ...
    Params, aliases, data all come from job files.
    :param vars: Variables/parameters in a list
    :param aliases: List of aliases for parameters as per jobfile.
    :param data: Data from job file.
    :return: result is a list of vars in correct dict form, names is the list of params
    :rtype: Tuple
    """
    result = []
    names = []
    for item in vars:
        ...
        if len(item) > 1:
            ...
            elif (item[1] == 'normal' or item[1] == 'lognormal')\
                    and len(item) > 2:
...
def process_inputs(config):
    """
    Get job details from the input file and update config with appropriate information.
    :param config: Dictionary of config settings
    :return: config dictionary updated with job details from input file
    :rtype: dict
    """
    ...
    # check for required inputs -- typically param and/or input may also be
    # needed, but it is conceivable that one might want to run a job without
    # them.
    for field in ['model', 'var']:
        if field not in job['header']:
            raise Exception(
                "field '%s' must be specified in the job file" % field)
    ...
    if tname not in timedata:
        if config['timestep']:
            ...
            raise Exception(
                "time step field '%s' not present in data file" % tname)
    ...
    config['baseSeq'], dummy = steps.readFiles(
        job['header'].get('init', [[]])[0])
    ...
    # for the moment the only supported distance functions are in the distance
    # module if that's ever not the case this could be a bit trickier...

    if config['sigma'] is not None and\
            job['header'].get('distance', [[DISTANCE]])[0][0] == 'loglik':
...
def make_optimiser(config, model):
    ...
    # run with dummy values -- this doesn't really belong here, but
    # temporarily...
    if config['wetrun']:
    ...
    if mode == 'GLP':
        ...
        return openopt.GLP(model.optimisable(config['distance'],
                                             weights=weights),
                           lb=lb,
                           ub=ub,
        ...
        lb = [x.get('min', 0) for x in config['params']]
        ub = [x.get('max', 0) for x in config['params']]
        return openopt.NLP(model.optimisable(config['distance'],
                                             weights=weights),
                           lb=lb,
                           ub=ub,
                           # TODO, possibly: support A and b args to define
                           # linear constraints
                           maxIter=config['max_iter'])
    elif mode == 'NSP':
        lb = [x.get('min', 0) for x in config['params']]
        ub = [x.get('max', 0) for x in config['params']]
        return openopt.NSP(model.optimisable(config['distance'],
                                             weights=weights),
                           lb=lb,
                           ub=ub,
                           # TODO, possibly: support A and b args to define
