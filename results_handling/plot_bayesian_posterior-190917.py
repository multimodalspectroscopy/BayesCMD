...
def scatter_dist_plot(df, params, frac, n_ticks, d=r'euclidean', verbose=False):
    ...
    with sns.plotting_context("talk", font_scale=1.2):
        g = sns.PairGrid(sorted_df, vars=params, hue='Accepted', diag_sharey=False)
...
def diag_kde_plot(x, **kws):
    ...
    ax.text(0.2, 0.9, "Median: {}".format(str(round_sig(x_median, 2))),transform=ax.transAxes)
...
def kde_plot(df, params,frac, plot_param=1, n_ticks=6, d=r'euclidean', verbose=False):
    ...
    for ii, ax in enumerate(g.axes.flat):
        for label in ax.get_xticklabels():
            label.set_rotation(50)
        ax.set_ylim(min(sorted_df[params[ii//len(params)]]), max(sorted_df[params[ii//len(params)]]))
        ax.set_xlim(min(sorted_df[params[ii%len(params)]]), max(sorted_df[params[ii%len(params)]]))
    plt.subplots_adjust(top=0.9)
...
def get_output(model_name, p, times, input_data, d0, targets, distance='euclidean', zero_flag=None):
    """
...
def plot_repeated_outputs(df, model_name, parameters, input_path, inputs, openopt_path, targets, n_repeats, frac,
    ...
    try:
        ...
        print("Number of requested model runs greater than posterior size:"
              "\n\tPosterior Size: {}\n\tNumber of runs: {}".format(posterior_size, n_repeats),file=sys.stderr)
        raise e
    ...
    while len(outputs_list) < n_repeats:
        ...
        output = get_output(model_name, p, times, input_data, d0, targets, distance='euclidean', zero_flag=None)
    ...
    with sns.plotting_context("talk", rc={"figure.figsize": (12, 9)}):
        ...
        for ii, target in enumerate(targets):
            ...
            true_plot, = ax[ii].plot(times, true_data[target], 'g', label='True')
