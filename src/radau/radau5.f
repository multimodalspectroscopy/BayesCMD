...
C -------- NMAX , THE MAXIMAL NUMBER OF STEPS -----
      ...
      ELSE
         NMAX=IWORK(2)
         IF (NMAX.LE.0) THEN
            WRITE(0,*)' WRONG INPUT IWORK(2)=',IWORK(2)
            ARRET=.TRUE.
         END IF
      END IF
C -------- NIT    MAXIMAL NUMBER OF NEWTON ITERATIONS
      ...
      ELSE
         NIT=IWORK(3)
         IF (NIT.LE.0) THEN
            WRITE(0,*)' CURIOUS INPUT IWORK(3)=',IWORK(3)
            ARRET=.TRUE.
         END IF
      END IF
...
C ------ HESSENBERG OPTION ONLY FOR EXPLICIT EQU. WITH FULL JACOBIAN
      IF ((IMPLCT.OR.JBAND).AND.IJOB.EQ.7) THEN
         WRITE(0,*)' HESSENBERG OPTION ONLY FOR EXPLICIT EQUATIONS WITH 
     &FULL JACOBIAN'
         ARRET=.TRUE.
      END IF
...
C ------ TOTAL STORAGE REQUIREMENT -----------
      ...
      IF(ISTORE.GT.LWORK)THEN
         WRITE(0,*)' INSUFFICIENT STORAGE FOR WORK, MIN. LWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
C --------- TOTAL REQUIREMENT ---------------
      ...
      IF (ISTORE.GT.LIWORK) THEN
         WRITE(0,*)' INSUFF. STORAGE FOR IWORK, MIN. LIWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
C --- FAIL EXIT
 176  CONTINUE
      WRITE(0,979)X   
      WRITE(0,*) ' MATRIX IS REPEATEDLY SINGULAR, IER=',IER
      IDID=-4
      RETURN
 177  CONTINUE
      WRITE(0,979)X   
      WRITE(0,*) ' STEP SIZE T0O SMALL, H=',H
      IDID=-3
      RETURN
 178  CONTINUE
      WRITE(0,979)X   
      WRITE(0,*) ' MORE THAN NMAX =',NMAX,'STEPS ARE NEEDED' 
      IDID=-2
      RETURN
